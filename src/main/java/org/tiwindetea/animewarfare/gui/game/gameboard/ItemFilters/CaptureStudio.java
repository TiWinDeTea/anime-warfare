 ...
 * @since 0.1.0
 */
public class CaptureStudio extends AbstractUnitFilter {
	@Override
	public List<MenuItem> apply(FactionType factionType, GUnit unit) {
		...
		MenuItem item = new MenuItem("Capture studio" + " (free)"); // todo: externalie
		item.setOnAction(e -> MainApp.getGameClient().send(new NetCaptureStudioRequest(unit.getZone(), unit.getGameID())));
		items.add(item);
