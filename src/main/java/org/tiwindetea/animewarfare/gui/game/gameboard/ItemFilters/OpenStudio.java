 ...
 * @since 0.1.0
 */
public class OpenStudio extends AbstractUnitFilter {
	@Override
	public List<MenuItem> apply(FactionType factionType, GUnit unit) {
		if (GamePhaseMonitor.getCurrentPhase() != PhaseChangedEvent.Phase.ACTION
				...
				|| GameLayoutController.getMap().getComponents(unit.getZone()).stream().anyMatch(c -> c instanceof GStudio)) {
		...
		MenuItem item = new MenuItem("Open studio" + " (" + cost + " SP)"); // todo: externalie
		item.setOnAction(e -> MainApp.getGameClient().send(new NetOpenStudioRequest(unit.getZone())));
		items.add(item);
